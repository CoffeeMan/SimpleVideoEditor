...
public class MyActivity extends Activity {
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        videoSaveBtn = findViewById(R.id.saveButton);

        progressDialog = new ProgressDialog(this);
        progressDialog.setTitle("Загрузка файла");
        //Метод инициализации видео
        initVideoView();
        //загрузка видео
        loadFFMpegBinary();
    }
            ...
            public void onPrepared(final MediaPlayer mp) {
                videoSliceSeekBar.setSeekBarChangeListener(new VideoSliceSeekBar.SeekBarChangeListener() {
                    ...
                    public void SeekBarValueChanged(int leftThumb, int rightThumb) {
                        //текст устанавливается в зависимости от выбранного тайминга
                ...
                videoSliceSeekBar.setRightProgress(11000); //Устанавливаем начальное положение правой передвигалки на 10 сек
                videoSliceSeekBar.setProgressMinDiff((5000 * 100)/mp.getDuration()); //Минимальная разница 5 сек
                videoSliceSeekBar.setProgressMaxDiff((10000 * 100)/mp.getDuration());//Максимальная разница 10 сек

                videoControlBtn.setOnClickListener(new View.OnClickListener() {//Метод для паузы/воспроизведения видео
                    @Override
                ...
                videoSaveBtn.setOnClickListener(new View.OnClickListener() {//Метод для сохранения
                    @Override
                    public void onClick(View v) {
...
//                    Log.d(TAG, "ffmpeg : correct");
//                }
//            });
        } catch (Exception e) {
            Log.d(TAG, "Exception no control : "  + e);
        }
    }
    ...
    public static String getTimeForTrackFormat(int timeInMills, boolean display2DigitsInMinsSection) {
        ...
        String result;
        if (display2DigitsInMinsSection && minutes < 10) result = "0";
        else result = "";
        result += minutes + ":";
        if (seconds < 10) {
            result += "0" + seconds;
    ...
    public void executeTrimCommand(int startMs, int endMs) {
        ...
        try {
            ...
            execFFmpegBinary("-i " + src.getAbsolutePath() + " -ss "+ startMs/1000 + " -to " + endMs/1000 + " -c copy "+ dest.getAbsolutePath());
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
